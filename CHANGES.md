...
## G+DON-1110-20200901-01
Fix:
* 一部のUTF-8絵文字が正しく表示されないのを修正。
* InstanceTickerが表示されなくなっていたのを修正。

Update/New:
* マストドンサーバーの設定によりローカル・連合の読み込みの挙動を切り替えるよう変更。
* アカウントのマストドンサーバー名から各サーバーの詳細ページに飛べるようリンク追加。


## G+DON-1109-20200809-01
Fix:
* 動画が投稿されるとタイムラインが表示されない場合があるのを修正。

Update/New:
* Mastodon ver 3.x以降のstatuses/:id/cardが廃止になっていたのに対応


## G+DON-1108-20200425-01
Fix:
* タイムラインのページでの更新ボタンが正常に機能しなかったのを修正

Update/New:
* Mastodon ver 3.x以降のstatuses/:id/cardが廃止になっていたのに対応


## G+DON-1107-20190912-01
Fix:
* PCのChrome（77.0.3865.75（Official Build） （64 ビット））だと表示が崩れて正常にデータを読み込めなくなっていたのを緊急で修正。

Update/New:
* セキュリティ用のライブラリを最新版に更新。


## G+DON-1106-20190805-01
Fix:
* ページ読み込み時の内部動作のセキュリティ強化
* 自分のトゥートの詳細メニューが開けなくなっていたのを修正。

Update/New:


## G+DON-1105-20190805-01
Fix:
* タイムラインを大量に読み込んだときに動作が異常に重くなる事態について対策（第1弾）

Update/New:
・タイムラインの読み込み時、常に表示される分を設定項目「タイムラインで常に保持するトゥート数」の数*2で維持するように変更。
* 上記の設定数からあふれた分のトゥートは画面から非表示にし、スクロールするとその分が再表示されるようにした。
* 画面の最上部へ手動でスクロールする際、新着トゥートの通知があっても新着トゥートは読み込まずに、まずは読み込み済みのトゥートを優先して表示するように変更。
* トゥート入力画面に「Googleフォトからメディアファイルを追加」ボタンを追加。
* 「Googleドライブからメディアファイルを追加」のアイコンを変更。
* アプリケーション設定の「タイムラインの列数」を「1列」にした場合のトゥートの幅を全体の60％程度に制限した。
